...
- 各角色对象：至少包含
  ...
  - `健康状况`（由数值映射：健康/亚健康/生病受伤/重病濒死/死亡）
  - `登场状态`（`登场` 或 `离场`）

### 2.2 推荐的 MVU 读写方式：`defineMvuDataStore`
...
5. **只处理 `登场状态 === '离场'` 的角色**
6. **如果 AI 在本轮改动了该角色的 `健康/健康更新原因/健康状况` 任一字段**，脚本视为“剧情介入”，本轮不接管该角色（避免冲突）
7. 写回字段：
   - `健康`
   - `健康更新原因`（标准格式）
   - `健康状况`（由数值重算）
8. 控制台日志：当实际 delta != 0 时输出
...
1. **离场健康没变**：
   - 检查本轮是否推进了 `世界.日期/时间`
   - 检查角色是否为 `登场状态: 离场`
   - 检查 AI 是否在本轮改过该角色的 `健康/健康更新原因/健康状况`（改过则脚本跳过）
2. **房间庇护没生效**：
